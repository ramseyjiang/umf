...
describe("Home component test", () => {
  ...
  it("Home displays correct", () => {
    ...
    expect(
      screen.getByText(/Allow Signed Up Users to be able to login/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Allow Logged in users to log out./i)
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/Fields you should have against the User table are/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /The fields created_at, updated_at should record Date and time /i
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Rebuild the Laravel auth default/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Deploy codes on AWS for preview and upload codes to GitHub for review./i
      )
    ).toBeInTheDocument();
  });
});
